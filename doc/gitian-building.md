...
- [Building Bluecoin Core](#building-bluecoin-core)
...
Clone the git repositories for bluecoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bluecoin-project/bluecoin
git clone https://github.com/bluecoin-project/gitian.sigs.ltc.git
```
...
in the bluecoin repository under 'Fetch and create inputs' to install sources which require
...
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bluecoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/bluecoin-project/bluecoin
...
For example:
```bash
URL=https://github.com/thrasher-/bluecoin.git
...
and the bluecoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bluecoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bluecoin
SIGPATH=/some/root/path/bluecoin-detached-sigs

./bin/gbuild --url bluecoin=${BTCPATH},signature=${SIGPATH} ../bluecoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bluecoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bluecoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bluecoin-osx-build.assert
```
...
[bluecoin-project/gitian.sigs.ltc](https://github.com/bluecoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
